...
export default class MainView extends Component<Props, State> {
  ...
  UNSAFE_componentWillReceiveProps(nextProps: Props) {
    if (!this.state.redirected) {
      ...
      if (
        isLoaded(nextProfile) &&
        isEmpty(nextProfile) &&
        ["/userinfo"].indexOf(this.props.location.pathname) !== -1 &&
        !isLoggedIn
      ) {
  ...
  changeLanguage(newLanguage: string) {
    ...
    console.log("IMPLEMENT WITH REACT-INTL");
    {
      // TODO Implement with react-intl
  ...
  isUserInfoOk() {
    ...
    const emailOk =
      (!profile.useOtherEmail && (providerEmail || profile.email)) ||
      (profile.useOtherEmail && profile.otherEmail);
    const namesOk =
      !!_.get(profile, "firstName", false) &&
      !!_.get(profile, "lastName", false);
    const everythingOk =
      !isLoggedIn || (emailOk && namesOk && acceptedPrivacyPolicy);
    return everythingOk;
  }
    ...
    return (
      <div>
        ...
        {!printPage && (
          <>
            <TitleBar
              returnToFrontpage={() => this.props.history.push("/today")}
            />
        ...
        {isLoaded(settings) && !printPage && <FooterBar />}
      </div>
    );
  }
}
