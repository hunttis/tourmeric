...
export const ParticipantList = ({ participations, maxParticipants }) => (
  <div>
    {participations && participations.map((participation, index) => {
      ...
      return (
        <div className="columns is-mobile" key={`participantModal-${participation.userId}`}>
          <div className={`column is-narrow is-mobile has-text-left is-fixed-bottom commenter ${maxParticipants !== 0 && maxParticipants < (index + 1) && 'has-text-warning'}`}>
