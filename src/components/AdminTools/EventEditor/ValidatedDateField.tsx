...
> {
  ...
  constructor(props: Props) {
    ...
    const viewTime =
      props.defaultValue && props.defaultValue.length === 10
        ? moment(props.defaultValue, "YYYY-MM-DD")
        : moment();
  ...
  parseInformationForMonthYear(month: number, year: number): Day[] {
    ...
    for (let i = 1; i <= dayCount; i += 1) {
      const dayString = `${_.padStart(String(i), 2, "0")}-${targetMonth.format(
        "MM-YYYY"
      )}`;
  ...
  render() {
    ...
    const eventEndDate = defaultEndValue
      ? moment(defaultEndValue, "YYYY-MM-DD")
      : null;
    ...
    const calendarData: Day[] = this.parseInformationForMonthYear(
      viewDate.month(),
      viewDate.year()
    );
    ...
    return (
      <div className="field is-horizontal">
        <div className="field-label is-normal">
          <label
            className={`label has-icons-left ${saved &&
              "has-text-success"} ${editing && "has-text-warning"} ${!isOk &&
              "has-text-danger"}`}
          >
            {saved && (
              <span className="icon is-small is-left">
