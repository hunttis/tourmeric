...
interface Props {
  chunkedCalendar: Day[][];
  clickDay: (key: string) => void;
  openinghoursexceptions: { [key: string]: OpeningHoursException };
  settings: Settings;
  selectedDay: string | null;
  selectedEndDay: string | null;
}

export const SmallCalendarDatePicker = ({
  chunkedCalendar,
  clickDay,
  ...
  <>
    {chunkedCalendar.map((week, weekIndex) => (
      <div
        ...
        className="column is-12 columns is-marginless is-paddingless is-mobile"
      >
        {week.map((day, dayIndex) => {
          ...
          const selected = selectedDay
            ? dayMoment.isSame(moment(selectedDay, "YYYY-MM-DD"), "day")
            : false;
          const isBetweenDates =
            selectedDay && selectedEndDay
              ? dayMoment.isBetween(
                  moment(selectedDay, "YYYY-MM-DD"),
                  moment(selectedEndDay, "YYYY-MM-DD"),
                  "day",
                  "()"
                )
              : false;
          const selectedEnd = selectedEndDay
            ? dayMoment.isSame(moment(selectedEndDay, "YYYY-MM-DD"), "day")
            : false;
          ...
          const englishDayName = moment(
            dayMoment.format("DD-MM-YYYY"),
            "DD-MM-YYYY"
          )
            .format("dddd")
            .toLowerCase();
            ...
            >
              <div
                className={`card calendar-day ${dayClass} ${closedThisDay &&
                  "strikeover"} ${(selected || selectedEnd) &&
                  "has-background-info"} ${isBetweenDates &&
                  "has-background-success"}`}
                onClick={() => {
                  clickDay(day.dayLink!);
                }}
              >
                <div className="card-header">
                  <div className="card-header-title calendar-title">
                    <div className="level calendar-cardtitle">
                      <div
                        className={`level-left ${selected && "has-text-black"}`}
                      >
                        {day.day}
                      </div>
                      <div
                        className={`level-right is-marginless has-text-right ${
                          selected ? "has-text-black" : "has-text-info"
                        }`}
                      >
